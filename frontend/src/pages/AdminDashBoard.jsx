...
function Toast({ toasts, remove }) {
  return (
    <div style={{ position:"fixed", bottom:"24px", right:"24px", zIndex:9999, display:"flex", flexDirection:"column", gap:"10px", alignItems:"flex-end" }}>
      {toasts.map(t => (
        <div key={t.id} onClick={() => remove(t.id)} style={{ background:"#0f1020", border:`1px solid ${t.type==="error"?"rgba(244,63,94,0.25)":"rgba(167,139,250,0.25)"}`, color:t.type==="error"?"#fda4af":"#c4b5fd", padding:"13px 18px", borderRadius:"13px", fontSize:"13px", fontWeight:600, display:"flex", alignItems:"center", gap:"10px", maxWidth:"90vw", boxShadow:"0 8px 40px rgba(0,0,0,0.6)", animation:"toastIn 0.3s ease", cursor:"pointer", fontFamily:"'Instrument Sans',sans-serif" }}>
...
function AdminDashboard() {
  ...
  const navItems = [
    ...
    { id:"leaves", icon:"🗓", label:"Leaves", bg:"rgba(236,72,153,0.1)", color:"#f472b6", badge: pendingLeaves },
  ];
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        :root{
          ...
          --pink:#ec4899;--text:#f0f0ff;--text2:#9898c0;--text3:#4a4a70;
        ...
        .adm-sb{width:260px;background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;flex-shrink:0;position:fixed;top:0;bottom:0;left:0;z-index:100;overflow-y:auto;box-shadow:4px 0 32px rgba(0,0,0,0.4);}
        ...
        .adm-lmark{width:40px;height:40px;background:linear-gradient(135deg,#7c3aed,#ec4899);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:900;color:white;flex-shrink:0;}
        ...
        .adm-nb-badge{margin-left:auto;background:#ec4899;color:white;font-size:9px;font-weight:700;padding:2px 7px;border-radius:100px;}
        ...
        .adm-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:22px;}
        ...
        .adm-st-top{display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;}
        ...
        .adm-st-num{font-family:'Bricolage Grotesque',sans-serif;font-size:38px;font-weight:800;color:var(--text);letter-spacing:-2px;line-height:1;}
        .adm-st-lbl{font-size:11px;color:var(--text3);margin-top:6px;text-transform:uppercase;letter-spacing:0.8px;}
        .adm-grid2{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;}
        .adm-card{background:var(--surface);border:1px solid var(--border);border-radius:22px;padding:26px;box-shadow:var(--card-shadow);position:relative;overflow:hidden;}
        .adm-card-full{background:var(--surface);border:1px solid var(--border);border-radius:22px;padding:26px;box-shadow:var(--card-shadow);position:relative;overflow:hidden;animation:fadeUp 0.4s ease;}
        ...
        .adm-email-btn{padding:10px 20px;background:linear-gradient(135deg,#0ea5e9,#6366f1);color:white;border:none;border-radius:9px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Instrument Sans',sans-serif;transition:all 0.2s;}
        .adm-email-btn:disabled{opacity:0.5;cursor:not-allowed;}
        ...
        .del-btn{background:rgba(244,63,94,0.08);color:#fb7185;border:1px solid rgba(244,63,94,0.15);padding:5px 12px;border-radius:7px;cursor:pointer;font-size:11px;font-weight:600;white-space:nowrap;}
        ...
        .mob-drawer{display:none;position:fixed;top:0;left:0;bottom:0;width:280px;background:var(--surface);z-index:200;flex-direction:column;overflow-y:auto;box-shadow:8px 0 40px rgba(0,0,0,0.6);transform:translateX(-100%);transition:transform 0.3s ease;}
        .mob-drawer.open{transform:translateX(0);}
        .mob-drawer-head{padding:20px 16px 16px;background:linear-gradient(180deg,rgba(124,58,237,0.12) 0%,transparent 100%);border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;}
        ...
        .mob-nb.on{background:rgba(139,92,246,0.15);color:var(--v4);font-weight:700;}
        ...
        .mob-nb-badge{margin-left:auto;background:#ec4899;color:white;font-size:9px;font-weight:700;padding:2px 8px;border-radius:100px;}
        .mob-foot{padding:16px;border-top:1px solid var(--border);margin-top:auto;}
        .mob-out{display:flex;align-items:center;gap:10px;padding:12px 14px;border:none;background:rgba(244,63,94,0.06);border:1px solid rgba(244,63,94,0.12);color:#fb7185;border-radius:12px;cursor:pointer;font-size:14px;font-weight:600;width:100%;font-family:'Instrument Sans',sans-serif;}

        @media(max-width:900px){
          .adm-sb{display:none;}
          .adm-main{margin-left:0;padding:16px 14px 80px;width:100%;}
          ...
          .adm-topbar{flex-direction:column;gap:4px;margin-bottom:16px;}
          .adm-ptitle{font-size:20px;}
          .adm-chip{display:none;}
          .adm-stats{gap:8px;}
          .adm-stat{padding:14px 10px;border-radius:16px;}
          .adm-st-num{font-size:26px;}
          .adm-st-ico{width:34px;height:34px;font-size:15px;}
          ...
          .adm-ch{flex-direction:column;gap:10px;}
          .adm-btn{width:100%;padding:13px;font-size:13px;}
          ...
          .sal-total{font-size:22px;}
        }
        @media(max-width:400px){
          .adm-st-num{font-size:22px;}
          .adm-st-ico{display:none;}
          .adm-main{padding:12px 10px 80px;}
        }
      `}</style>
      ...
      <div className={`mob-drawer ${menuOpen?"open":""}`}>
        ...
        <div style={{ padding:"12px 16px 8px", borderBottom:"1px solid var(--border)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:"10px", background:"rgba(139,92,246,0.08)", border:"1px solid var(--border2)", borderRadius:"12px", padding:"10px 13px" }}>
        ...
        <div className="mob-nav">
          {navItems.map(item => (
            <button key={item.id} className={`mob-nb ${activeTab===item.id?"on":""}`} onClick={() => goTo(item.id)}>
              ...
              {item.label}
              {item.badge > 0 && <span className="mob-nb-badge">{item.badge}</span>}
            </button>
          ))}
        </div>
          ...
          <div className="adm-nav-wrap">
            ...
            <nav className="adm-nav">
              {navItems.slice(0,3).map(item => (
                <button key={item.id} className={`adm-nb ${activeTab===item.id?"on":""}`} onClick={() => setActiveTab(item.id)}>
            ...
            <div className="adm-section">Payroll</div>
            <nav className="adm-nav">
        ...
        <div className="adm-main">
          ...
          {activeTab === "overview" && (
            <div className="adm-grid2">
              ...
              <div className="adm-card">
                <div className="adm-ch"><div><div className="adm-ct">Team Summary</div><div className="adm-cs">{employees.length} employees</div></div><button className="adm-btn" onClick={() => setActiveTab("add")}>+ Add New</button></div>
          ...
          {activeTab === "leaves" && (
            <div className="adm-card-full">
              <div className="adm-ch"><div><div className="adm-ct">Leave Requests</div><div className="adm-cs">{pendingLeaves} pending</div></div>{pendingLeaves>0&&<span className="badge" style={{ background:"rgba(244,63,94,0.1)",color:"#fb7185",border:"1px solid rgba(244,63,94,0.2)" }}>{pendingLeaves} Pending</span>}</div>
                ...
                <div style={{ overflowX:"auto", WebkitOverflowScrolling:"touch" }}>
                  <table className="adm-table" style={{ minWidth:"580px" }}>
                    ...
                    <tbody>{leaves.map(lv => {
                      ...
                      return (
                        <tr key={lv._id}>
                          <td className="emp-n">{lv.employeeName}</td><td>{lv.type}</td>
